...
export default class ClubService extends BaseService {
  ...
  async getOwnerClubByShortId(playerShortId) {
    const player = await service.playerService.getPlayerModelByShortId(playerShortId);
    if (!player) {
      return null;
    }
    return this.getOwnerClub(player._id);
  }
  ...
  async calculateGold(clubShortId, playerId, goldAmount) {
    ...
    const ownerMember = await this.getOwnerMember(unionClub._id);
    if (!ownerMember) {
      return result;
    }
    result.inviterPlayerId = unionClub.owner;
    result.inviterGold = service.utils.accMul(goldAmount, ownerMember.degree);
    return result;
  }
