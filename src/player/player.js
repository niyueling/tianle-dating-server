...
const isTokenValid = async (apiName, token, player) => {
  ...
  if (!config.jwt.needNotToken.includes(apiName)) {
    ...
    const { isOk, data } = await verifyWithRecord(token, player.model._id);
...
class Player extends EventEmitter {
  ...
  get gem() {
    return (this.model && this.model.gem) || 0;
  }
  ...
  async updateResource2Client() {
    ...
    this.sendMessage('resource/update', pick(model, ['gold', 'gem', 'ruby']))
  }
