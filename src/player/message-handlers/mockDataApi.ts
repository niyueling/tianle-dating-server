...
import { robotAvatars } from "@fm/common/constants";
...
export class MockDataApi extends BaseApi {
  ...
  @addApi()
  async saveShopData() {
    const result = await BlockShop.find();

    if (result.length) {
      await BlockShop.remove({_id: {$ne: null}}).exec();
    }
    // 新手宝箱
    const newGift = {
      name: "新手宝箱",
    ...
    await BlockShop.create(newGift);

    // 普通宝箱，prizeLists包含多个数组，表示奖励随机，并且金币奖励number为数组表示区间，普通宝箱有1-10级
    const normalGift = [{
      name: "普通宝箱",
      type: 1,
      giftType: 2,
    ...
    await BlockShop.insertMany(normalGift);
    ...
    await BlockShop.insertMany(debrisGift);
    ...
    await BlockShop.insertMany(goldGift);
    ...
    await BlockShop.create(diamondGift);
    ...
    await BlockShop.create(powerGift);

    return this.replySuccess({newGift, normalGift, debrisGift, goldGift, diamondGift, powerGift});
  }
  ...
  async saveActiveGift() {
    ...
    if (result.length) {
      await BlockTurntablePrize.remove({_id: {$ne: null}}).exec();
    }

    const gifts = [
        {probability: 0.2, num: 500, residueNum: 10000, type: 2, propId: 103},
        {probability: 0.04, num: 5, residueNum: 10000, type: 4, propId: 119},
        {probability: 0.3, num: 100, residueNum: 10000, type: 3, propId: 102},
        {probability: 0.01, num: 5, residueNum: 10000, type: 4, propId: 120},
        {probability: 0.2, num: 5, residueNum: 10000, type: 1, propId: 101},
        {probability: 0.04, num: 5, residueNum: 10000, type: 4, propId: 115},
        {probability: 0.2, num: 50, residueNum: 10000, type: 3, propId: 102},
        {probability: 0.01, num: 2, residueNum: 10000, type: 4, propId: 117}
    ];

    BlockTurntablePrize.insertMany(gifts);
  ...
  @addApi()
  async saveRobots() {
    const result = await BlockUser.find({robot: true});

    if (result.length) {
      await BlockUser.remove({robot: true}).exec();
    }
    ...
    try {
      ...
      for (let i = 0; i < profiles.length; i++) {
        ...
        const exist = await BlockUser.count({robot: true, openid: data.openid});

        if (exist) {
          continue;
        }

        data.avatar = robotAvatars[i];
        data.robot = true;
        data.shortId = shortId + i + 1;
        data.curLevel = random(3, 72);
        ...
        console.log(data.ip)
        const res = await service.playerService.getLocation(data.ip);

        if (res.code === 200) {
          data.province = res.data.result.prov;
          data.city = res.data.result.city;
        }
