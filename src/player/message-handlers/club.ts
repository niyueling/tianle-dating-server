...
export async function getClubInfo(clubId, player?) {
  ...
  return {
    ok: true,
    data: {roomInfo: room, clubInfo, unReadMessage, clubs, isAdmin: !!isAdmin, isPartner: !!isPartner, isClubOwner}
  };
}
...
export default {
  ...
  [ClubAction.getInfo]: async (player, message) => {
    ...
    return player.replySuccess(ClubAction.getInfo, {
      roomInfo: room,
      clubInfo,
      unReadMessage,
      clubs,
  ...
  [ClubAction.getClubPartner]: async (player, message) => {
    ...
    player.sendMessage('club/getClubPartnerReply', {
      ok: true,
      data: {clubPartnerInfo, total: {...gameCount, totalGameJuCount}}
    });
  },
