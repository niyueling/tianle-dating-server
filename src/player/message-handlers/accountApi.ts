...
export class AccountApi extends BaseApi {
  ...
  async benefitData() {
    ...
    if (user.helpCount > 0) {
      ...
      return this.replySuccess({
        ...
        totalCount: user.helpCount + helpCount,
        vipLevel: user.vip,
        benifitMultiple: vipConfig.benifitMultiple
      });
    }
  ...
  async benefit(message) {
    ...
    if (user.helpCount > 0) {
      ...
      return this.replySuccess({
        gold: gold,
        helpCount: helpCount + 1,
        totalCount: user.helpCount + helpCount + 1,
        vip: user.vip
      });
    }
  ...
  async updateUserInfo(msg) {
    ...
    if (msg.mnpVersion) {
      ...
      if (msg.platform && msg.platform === "iOS") {
        ...
        model.openIosShopFunc = openIosShopFunc && iosRoomCount >= 3 && iosLotteryCount >= 2 && !isTest;
      }
    }
  ...
  async getActivityInfo(user, mnpVersion, platform) {
    ...
    // 判断7日签到是否开放
    const sevenLoginCount = await SevenSignPrizeRecord.count({
      playerId: user._id,
      createAt: {$gte: start, $lt: end}
    });

    // 判断开运红包是否开放
    const startPocketCount = await StartPocketRecord.count({
      playerId: this.player.model._id,
      createAt: {$gte: start, $lt: end}
    });
    ...
    // 判断新手签到是否可领取
    const todayReceiveCount = await NewSignPrizeRecord.count({
      playerId: user._id,
      createAt: {$gte: start, $lt: end}
    });
    ...
    const privateMails = await MailModel.find({
      to: user._id,
  ...
  async getBackPackByHeader() {
    ...
    for (let i = 0; i < lists.length; i++) {
      const playerHeadBorder = await PlayerHeadBorder.findOne({
        playerId: this.player._id,
        propId: lists[i].propId
      });
