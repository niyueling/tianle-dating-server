...
export class AccountApi extends BaseApi {
  ...
  async saveGoodsReviveTlGolds() {
    const result = await GoodsReviveSupplement.find();

    if (result.length) {
      await GoodsReviveSupplement.remove({_id: {$ne: null}}).exec();
    }

    const datas = [
      {level: 1, price: 6, originPrice: 22, gold: 2000000000, todayReceiveLimit: 3, maxReceiveDayLimit: 7, todayReceiveGold: 200000, gameType: "majiang"},
    ];

    await GoodsReviveSupplement.insertMany(datas);

    this.player.sendMessage("account/saveGoodsReviveTlGoldsReply", {ok: true, data: datas});
  }
