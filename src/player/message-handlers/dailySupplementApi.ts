...
import DailySupplementGiftRecord from "../../database/models/DailySupplementGiftRecord";
import DailySupplementReceiveRecord from "../../database/models/DailySupplementReceiveRecord";

export class DailySupplementApi extends BaseApi {
  // 福利包列表
  @addApi()
  async dailySupplementLists() {
    ...
    const data = await this.getDailySupplementLists(user);
  ...
  async getDailySupplementLists(user) {
    const giftList = await DailySupplementGift.find().lean();
    let gifts = [];

    for (let i = 0; i < giftList.length; i++) {
      // 每日福利包领取次数
      giftList[i].receiveCount = 0;

      // 判断用户是否购买
      const payInfo = await DailySupplementGiftRecord.findOne({playerId: user._id, prizeId: giftList._id, payStatus: true});

      // 如果没有付费记录
      if (!payInfo) {
        giftList[i].payStatus = false;
        continue;
      }

      // 如果支付超过7天，则支付失效
      const payTime = Date.parse(giftList[i].createAt);
      const startTime = new Date().getTime() - 1000 * 60 * 60 * 24 * 7;

      if (payTime < startTime) {
        giftList[i].payStatus = false;
        continue;
      }

      giftList[i].payStatus = true;

      // 计算今日领取次数
      const start = moment(new Date()).startOf('day').toDate()
      const end = moment(new Date()).endOf('day').toDate()
      giftList[i].receiveCount = await DailySupplementReceiveRecord.count({playerId: user._id, prizeId: giftList._id, createAt: {$gte: start, $lt: end}});

      gifts.push(giftList[i]);
    }
  ...
  async getFirstRechargeList(user) {
    ...
    const taskList = await NewFirstRecharge.find().sort({day: 1}).lean();
